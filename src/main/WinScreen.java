...
public class WinScreen {
		...
		for(int i = 0; i < bounds.length; i++) {
			if(bounds[i].contains(Controller.mousePoint) && MouseHandler.MOUSEDOWN) {
				...
				if(i == 0) {
					...
					BgmLoader.StopBGM();
					if(Controller.level + 1 == 1) BgmLoader.BGM(BgmLoader.Lvl1_BGM);
					if(Controller.level + 1 == 2) BgmLoader.BGM(BgmLoader.Lvl2_BGM);
					if(Controller.level + 1 == 3) BgmLoader.BGM(BgmLoader.Lvl3_BGM);
					if(Controller.level + 1 == 4) BgmLoader.BGM(BgmLoader.Lvl4_BGM);
					if(Controller.level + 1 == 5) BgmLoader.BGM(BgmLoader.Lvl5_BGM);
					if(Controller.level + 1 == 6) BgmLoader.BGM(BgmLoader.Lvl6_BGM);
					if(Controller.level + 1 == 7) BgmLoader.BGM(BgmLoader.Lvl7_BGM);
					BgmLoader.PlayBGM();
	...
	public void render(Graphics g) {
		...
		if(Controller.level < Level.MAX_LEVEL-1) {
			...
			g.drawString("LEVEL " + Controller.level + " COMPLETE", Frame.WIDTH/2-g.getFontMetrics().stringWidth("LEVEL COMPLETE")/2, 150);
			g.drawString("Score: " + Controller.score, Frame.WIDTH/2-g.getFontMetrics().stringWidth("Score: " + Controller.score)/2, 190);
			...
			if(unlockedNewLevel) {
				...
				g.drawString("NEW LEVEL UNLOCKED!", Frame.WIDTH/2-g.getFontMetrics().stringWidth("NEW LEVEL UNLOCKED!")/2, 100);
				g.setColor(Color.white);
			}
			for(int i = 0; i < bounds.length; i++) {
			...
			g.drawString("Next Level", (Frame.WIDTH/2-g.getFontMetrics().stringWidth("Next Level")/2) - 3, mainMenu.y-110);
			g.drawString("Main Menu", Frame.WIDTH/2-g.getFontMetrics().stringWidth("Main Menu")/2, mainMenu.y+40);
			g.drawString("Retry", (Frame.WIDTH/2-g.getFontMetrics().stringWidth("Retry")/2) - 3, mainMenu.y-35);
		}else{
			g.setColor(Color.orange);
			g.drawString("CONGRATULATIONS YOU BEAT THE GAME!", Frame.WIDTH/2-g.getFontMetrics().stringWidth("CONGRATULATIONS YOU BEAT THE GAME!")/2, 100);
			g.setFont(Controller.bigFont);
			g.drawString("Thank you for playing!", Frame.WIDTH/2-g.getFontMetrics().stringWidth("Thank you for playing!")/2, 150);
			g.setFont(Controller.smallFont);
			g.drawString("Please look forward for our next updates." , Frame.WIDTH/2-g.getFontMetrics().stringWidth("Please look forward for our next updates.")/2, 190);
			g.setColor(Color.white);
			g.drawRect(140, 235, 210, 45);
			g.drawRect(140, 310, 210, 45);
			for(int i = 0; i < 2; i++) {
			...
			g.drawString("Main Menu", Frame.WIDTH/2-g.getFontMetrics().stringWidth("Main Menu")/2, mainMenu.y-110);
			g.drawString("Retry", (Frame.WIDTH/2-g.getFontMetrics().stringWidth("Retry")/2) - 3, mainMenu.y-35);
		}
		for(int i = 0; i < options.length; i++) { 
